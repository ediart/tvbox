...
const getViews = async () =>
  fetchApi(buildUrl(`/emby/Users/${config.userId}/Views`));
...
const homeVod = async () => {
  const url = buildUrl(`/emby/Users/${config.userId}/Items`, {
    ...
    Limit: 40,
    Fields:
      'BasicSyncInfo,CanDelete,Container,PrimaryImageAspectRatio,ProductionYear,CommunityRating,Status,CriticRating,EndDate,Path,Overview',
...
const category = async (tid, pg = 1) => {
  ...
  const url = buildUrl(`/emby/Users/${config.userId}/Items`, {
    ...
    Recursive: 'true',
    Fields:
      'BasicSyncInfo,CanDelete,Container,PrimaryImageAspectRatio,ProductionYear,CommunityRating,Status,CriticRating,EndDate,Path',
  ...
  const json = await fetchApi(url);
  if (!json) return JSON.stringify({ list: [], page: +pg, pagecount: 1, limit: 30 });
...
const detail = async (id) => {
  const info = await fetchApi(
    buildUrl(`/emby/Users/${config.userId}/Items/${id}`, {
      Fields:
        'BasicSyncInfo,CanDelete,Container,PrimaryImageAspectRatio,ProductionYear,CommunityRating,Status,CriticRating,EndDate,Path,Overview,Genres,People,Taglines,Studios',
    }),
  );
  ...
  const directors =
    info?.People?.filter(
      (person) => person.Type === 'Director' || person.Role === 'Director',
    ).map((person) => person.Name) || [];
  const actors =
    info?.People?.filter(
      (person) => person.Type === 'Actor' || person.Role === 'Actor',
    ).map((person) => person.Name) || [];
  ...
  const VOD = {
    ...
    vod_content:
      info?.Overview?.replace(/\xa0/g, ' ').replace(/\n\n/g, '\n').trim() || '暂无简介',
    vod_year: year,
  ...
  if (info.Type === 'Series') {
    const seasons = await fetchApi(
      buildUrl(`/emby/Shows/${id}/Seasons`, {
    ...
    for (const season of seasons?.Items || []) {
      ...
      const episodes = await fetchApi(
        buildUrl(`/emby/Shows/${id}/Episodes`, {
          SeasonId: season.Id,
          ImageTypeLimit: 1,
          UserId: config.userId,
...
const play = async (_, id) => {
  ...
  const getPublicUrl = (originalUrl) =>
    originalUrl ? `${config.host}${originalUrl.replace(/^https?:\/\/[^\/]+/i, '')}` : '';
  let playUrl = '';
  if (mediaSource.DirectStreamUrl) playUrl = getPublicUrl(mediaSource.DirectStreamUrl);
  else if (mediaSource.DirectPlayUrl) playUrl = getPublicUrl(mediaSource.DirectPlayUrl);
  else return JSON.stringify({ parse: 1, msg: '无直通播放链接' });
  return JSON.stringify({ parse: 0, url: playUrl, header: getHeaders() });
};
